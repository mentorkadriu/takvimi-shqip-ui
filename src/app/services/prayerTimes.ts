...
function applyLocalAdjustments(times: PrayerTimes, cityName: string): PrayerTimes {
  ...
  const adjustmentToUse =
    cityAdjustment || (isKosovo ? LOCAL_ADJUSTMENTS.find((adj) => adj.city === 'Kosovo') : null);
...
export function getCurrentLocation(): Promise<Coordinates> {
  return new Promise((resolve, reject) => {
    ...
    if (locationCache.coordinates && Date.now() - locationCache.timestamp < LOCATION_CACHE_EXPIRY) {
